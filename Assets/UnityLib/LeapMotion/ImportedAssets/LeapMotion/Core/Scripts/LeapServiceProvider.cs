...
namespace Leap.Unity {
    ...
    protected int BounceAmount = 70;
        ...
        private float _fixedUnscaledTime;
        ...
        protected virtual void Update() {
            ...
            if (_fixedUnscaledTime + 0.02f < Time.unscaledTime) { //0.02f it's fixedDeltaTime. Dirty hardcode
                _fixedUnscaledTime = Time.unscaledTime;
                UnscaledFixedUpdate();
            }
#if UNITY_EDITOR
            ...
            _fixedOffset.Update(Time.unscaledTime - _fixedUnscaledTime, Time.unscaledDeltaTime);
            ...
            if (_useInterpolation) {
                ...
                _smoothedTrackingLatency.Update((float)(leap_controller_.Now() - leap_controller_.FrameTimestamp()), Time.unscaledDeltaTime);
#endif
                long timestamp = CalculateInterpolationTime() + (ExtrapolationAmount * 1000);
                _unityToLeapOffset = timestamp - (long)(Time.unscaledTime * S_TO_NS);
        ...
        protected virtual void UnscaledFixedUpdate() {
            if (_frameOptimization == FrameOptimizationMode.ReuseUpdateForPhysics) {
            ...
            if (_useInterpolation) {
                ...
                switch (_frameOptimization) {
                    case FrameOptimizationMode.None:
                        //By default we use Time.fixedUnscaledTime to ensure that our hands are on the same timeline
                        //as Update.  We add an extrapolation value to help compensate for latency.
                        float extrapolatedTime = _fixedUnscaledTime + CalculatePhysicsExtrapolation();
                    ...
                    case FrameOptimizationMode.ReusePhysicsForUpdate:
                        ...
                        //Time.fixedUnscaledTime, just grab the most recent interpolated timestamp like we are
                        //in Update
...
#else
            if (leap_controller_ != null) {
                return leap_controller_.Now() - (long)_smoothedTrackingLatency.value + (_updateHandInPrecull && !endOfFrame ? (long)(Time.unscaledDeltaTime * S_TO_NS / Time.timeScale) : 0);
            } else {
                return 0;
            }
#endif
